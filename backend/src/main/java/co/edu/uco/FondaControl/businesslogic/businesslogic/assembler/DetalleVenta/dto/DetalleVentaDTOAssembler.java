...
public final class DetalleVentaDTOAssembler implements DTOAssembler<DetalleVentaDTO, DetalleVentaDomain> {
	...
	public DetalleVentaDomain toDomain(final DetalleVentaDTO dto) {
		...
		return new DetalleVentaDomain(dto.getCodigoDetalleVenta(), dto.getNombreProducto(), dto.getPrecioAplicado(),
				dto.getCantidad());
	}
	...
	public DetalleVentaDTO toDto(final DetalleVentaDomain domain) {
		if (UtilObjeto.esNulo(domain)) {
			return DetalleVentaDTO.obtenerValorDefecto();
		}
		return DetalleVentaDTO.builder().codigoDetalleVenta(domain.getCodigoDetalleVenta())
				.nombreProducto(UtilTexto.getInstancia().obtenerValorDefecto(domain.getNombreProducto()))
				.precioAplicado(domain.getPrecioAplicado()).cantidad(domain.getCantidad()).crear();
	}
